...
for h=1:numReps
    ...
    for j = 1:nModels
        ...
        for i = 1:length(outcome)
            ...
            % to prediction error
            [errBased_pOdd,errBased_RU,errBased_LR,errBased_UP]=...
                getTrialVarsFromPEs_cannon([outputStd; outputStd],...
                [PE; 0], Haz, [1; 0], [false, false], ...
        ...
        if j == 52
            ...
            a=[0 0.25 0.50 0.75 2];
            for t=1:4
                prob=and((a(t+1)>pOdd),(a(t)<=pOdd));
                C2 = regress(UP(prob),xes(prob,:));
                postOBtrialLR(j,t)=C2(2);
            end
        end
    ...
    contextShiftLROB(h,:)=postOBtrialLR(52,:);
...
if showPlots
    ...
    l=legend('Outcome','dynamic learning rate ','dynamic changepoint','dynamic latent states');
    ...
    xlabel('context Shifts')
    xticks(1:4)
    xticklabels({'0-0.25','0.25-0.50','0.50-0.75','>0.75'})
